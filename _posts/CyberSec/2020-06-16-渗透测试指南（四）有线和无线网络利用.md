...
# 基于网络的漏洞利用
本章节从一些典型的网络服务出发，旨在说明基于网络的漏洞利用的基本思路和方法
...
## NetBIOS，LLMNR和SMB
NetBIOS和LLMNR是微软用于主机识别的协议。打开windows系统 控制面板--系统 界面，可以看到“计算机名称，域和工作组设置”就可以看到计算机名，比如Pentest，这个名字就和域名一样，可以用来替换难记的ip地址。举一个简单的例子，比如右键共享某个文件夹xxx，共享完成后，系统会提示通过访问 \\Pentest\xxx 查看共享文件。通过共享也就开启了SMB服务

和NetBIOS有关的端口包括TCP 135，139；UDP 137，138。和SMB有关的端口为TCP 445。在访问
