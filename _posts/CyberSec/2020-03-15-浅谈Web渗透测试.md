...
在我的文章 [OWASP 安全测试指南解读](https://kafroc.github.io/2020/02/15/OWASP%E5%AE%89%E5%85%A8%E6%B5%8B%E8%AF%95%E6%8C%87%E5%8D%97%E8%A7%A3%E8%AF%BB.html) 中，我解读了 OWASP 的 Web 渗透测试方法论。
方法论总是高高在上，本篇文章我想谈谈 渗透测试人员怎么把 OWASP 测试方法论落地。

在此之前，我想先谈谈我对 渗透测试 和 漏洞挖掘 这两个概念的理解。
...
可能对于不是很懂行的甲方来说，渗透测试的核心就是漏洞列表及详情，好像上述三类报告并没太大差别。

实际上并不是这样的，一个非常有组织的渗透测试，报告的内容可以非常丰富。

### 漏洞挖掘

漏洞挖掘 是以漏洞为导向，每年大量的 CVE 就是漏洞挖掘的结果，比如打 CTF 也是在做漏洞挖掘及利用的过程。毫无疑问 漏洞挖掘能力 能表现安全人员的技能水平。

举个例子来说，假设某天 xxx SRC 发布一个公告，中低危漏洞不再收录，只收高危及以上。你作为一个白帽子，像用户名暴破这样的漏洞看都不看就跳过，但是如果你是在做渗透测试，就不能这么轻易的忽略这些漏洞。

### 渗透测试

渗透测试更侧重过程及方法，测试结果只是过程的产物。总体而言，渗透测试的目标是通过结构化的渗透测试方法，定位系统存在的所有脆弱点(Vulnerability)，如果是漏洞评估到这里就结束了。

如果是渗透测试，还需要对这些脆弱点进行进一步利用，甚至是留后门，清除痕迹等。也就是 [PTMS](http://www.pentest-standard.org/index.php/Main_Page) 里说的 Exploitation 和 Post Exploitation 这两个过程。

所以任何的脆弱点在测试范围之内都不能被漏掉的，小到敏感信息明文传输，用户名暴破，路径信息泄漏，大到 SQL 注入，认证绕过，越权等等。

### 个人看法

我个人认为 渗透测试和漏洞挖掘的关系 互相不是充要条件。就是说一个人很懂渗透测试工程方法，不一定很会挖漏洞。一个很会挖漏洞的人也不一定能做好渗透测试。

但总体而言，漏洞挖掘的门槛比渗透测试方法会更高。一个非常优秀的 CTFer，应该可以很快掌握渗透测试的精髓，反之一个人精通渗透测试方法论，不一定能快速掌握漏洞挖掘技巧。
...
在我看来 模式二 更花时间精力，资费应该更贵，对甲方也更有价值。模式一 对乙方团队来说，多劳多得，压力也会小些。

模式二真正的问题是甲方怎么样才能信任乙方团队，一方面乙方团队有没有这个能力来接渗透测试的活。另一方面 怎么才能保证乙方团队的数据保密性。

结果也很明显，业界更倾向于模式一，要么自己建立 SRC，收漏洞，根据漏洞危害等级付相应酬劳。要么和大的白帽平台合作，在这些平台上发起众测。

我还是觉得模式二对乙方更有价值，很多小型公司没有足够资金预算投入人力做安全。可以雇佣 安全团队 做风险评估/渗透测试等。 SECaaS（security as a service）理论上还是蛮好的。

## Web 渗透测试方法构想

本章 以 PTES（见参考文献[1]）和 OWASP 测试指南（见参考文献[2]） 为基础，构建我的 Web 渗透测试落地方法
...
把渗透利用和后渗透合并是因为，很多时候我感觉渗透测试并不是太强调渗透后的 维护/清理，事实上甲方也不是很喜欢你在报告中说可以种一个远程访问木马这样的话。

## 前期交互

前期交互的核心是 范围和目标。范围是指测试覆盖的范围, 涉及到的资产（服务器，域名/ip，数据库 等等）。
...
输出件：根据 测试范围，测试目标，测试时间/进程安排等 整理出测试计划文档
...
- GET /main/asdf
- POST /subproc/fdsa<br><br>{xxx=xyz}
- PUT /upload/tmpfile<br><br>file=fake content
...
这要看具体的场景，如果甲方在产品中有部署防火墙，WAF 等，甲方想知道在这种环境下，即使有 SQL 注入漏洞，能造成什么危害。是不是 WAF 能识别所有攻击行为。虽然有漏洞也是安全的。还是说可能有多个漏洞串联造成大危害。这些都是漏洞分析不会去做的。
...
输出件：结合上面几个环节输出件，共输出 测试计划文档一份，漏洞 Checklist 一份，与 checklist 及测试范围匹配的 测试用例一份，包含漏洞详情的测试报告一份
...
本次以 8/9 题（Ticketastic: Demo Instance/Ticketastic: Live Instance）为测试目标，演示如果利用上述的 Web PTES 进行渗透测试。

### 信息收集

接口 /newTicket， /login， /admin， /ticket， /newUser

测试范围：http://35.190.155.168/b9b2ddf96c/ <br>
测试目标：用户数据库
...
对 表 6 漏洞 Checklist 进行逐个漏洞分析排查，排查过程中，整理测试用例
...
执行 漏洞分析 过程之后，掌握来多个漏洞之后，就要利用这些漏洞，可能是单个漏洞的利用，也可能是组合利用
...
- 思路 1: 破解 session <br>
- 思路 2: 通过 xss 窃取 cookie<br>
- 思路 3: 通过 CSRF 添加账户<br>
- 思路 4: 暴力破解密码
...
都没有成功，所以本题我也没解出来，导致另外一个 FLAG 也拿不到。
...
上面这些过程中，每个过程都是操作者是强相关的。不同的人，在漏洞建模环节会输出不同的漏洞 checklist，会写出不同的用例，会得到不同的漏洞列表。所以这只是一套落地渗透测试方法论的方法。

以上就是我个人对 Web 渗透测试执行的一些想法。到这里差不多就结束了。其实我用这个环境也只是想演示渗透测试流程，不是 CTF 教程。所以 GAME OVER.

**因笔者水平有限，文章难免会有错误，欢迎读者批评指正。笔者个人邮箱：kafrocyang@gmail.com**
